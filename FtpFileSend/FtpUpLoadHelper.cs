...
{
    public abstract class FtpUpLoadHelper
    {
        //FTP的服务器地址，格式为ftp://192.168.1.234:8021/。ip地址和端口换成自己的，这些建议写在配置文件中，方便修改
        private static string FTPCONSTR = ConfigHelper.FTPAddress;
        private static string FTPUSERNAME = ConfigHelper.FTPName;//FTP服务器的用户名
        private static string FTPPASSWORD = ConfigHelper.FTPPwd;//FTP服务器的密码
        ...
        public static bool UploadFile(string path, string name)
        {
            ...
            {
                string erroinfo = string.Format("因{0},无法完成上传", ex.Message);
                LogHelper.DoErrorLog(erroinfo);
                return false;
            }
        }
